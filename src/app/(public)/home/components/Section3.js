  ...
  return (
    <section className="w-full bg-[#FCFBFA] font-sans antialiased">
      ...
      <div className="max-w-350 mx-auto px-2 sm:px-6 -mt-12 relative z-10 pb-16">
        ...
        <div className="bg-white rounded-2xl border border-[#F2EEE8] shadow-[0_4px_25px_rgba(0,0,0,0.06)] p-3 mt-5">
          <div className="flex flex-col lg:flex-row lg:items-center gap-3">
            <div className="bg-[#FAFAF9] flex gap-3 items-center h-12 rounded-lg px-4 border border-[#ECE7DE] text-slate-500 text-sm lg:flex-1">
            ...
            <div className="grid grid-cols-2 gap-3 w-full lg:flex lg:gap-3 lg:items-center lg:w-auto">
              ...
              <button
                type="button"
                className="w-12 h-12 rounded-lg border border-[#D9B577] flex justify-center items-center text-[#C59A52] hover:bg-[#B68A35]/5"
              >
        ...
        <div className="rounded-2xl mt-5 border border-[#F2EEE8] shadow-[0_4px_25px_rgba(0,0,0,0.06)] bg-white overflow-hidden">
          <div className="flex border-b border-[#F2EEE8] bg-[#FAF9F6]">
            <button
              onClick={() => setActiveTab('byDeveloper')}
              className={`flex-1 px-4 py-4 font-semibold text-center transition-colors ${activeTab === 'byDeveloper'
            ...
            <button
              ...
              className={`flex-1 px-4 py-4 font-semibold text-center transition-colors ${activeTab === 'allProjects'
                ? 'text-[#B68A35] border-b-2 border-[#B68A35] bg-white'
                : 'text-slate-600 hover:text-slate-900'
                }`}
            >
          ...
          {activeTab === 'byDeveloper' ? (
            <div className="bg-white max-h-96 overflow-y-auto custom-scroll">
              {locations.map((item, idx) => {
                ...
                return (
                  <div key={item.name} className="bg-[#FAF9F6] border-b border-[#F2EEE8] last:border-b-0">
                    <button
                      type="button"
                      onClick={() => setActiveIndex(isOpen ? null : idx)}
                      className={`w-full p-2 flex gap-4 items-center text-left transition-colors ${isOpen ? 'bg-[#FAF9F6]' : 'bg-white hover:bg-[#FCFAF5]'
                        }`}
                    >
                      <div className="w-12 h-12 sm:w-12 sm:h-12 bg-white rounded-xl flex justify-center items-center border border-[#EFE8DC] shrink-0">
                        <Icon className="text-[#B68A35] w-6 h-6 sm:w-7 sm:h-7" />
                      </div>

                      <div className="flex justify-between items-center flex-1 min-w-0">
                        <div className="min-w-0">
                          <h3 className="font-semibold text-base sm:text-md text-slate-900 truncate">{item.name}</h3>
                          <p className="text-sm text-slate-500 truncate">{item.subtitle}</p>
                        </div>

                        <div className="flex gap-3 items-center pl-3">
                          <div className="bg-[#B68A35] min-w-8 h-6 px-2 sm:min-w-10 sm:h-7 text-white rounded-full flex justify-center items-center text-xs sm:text-sm">
                            {projects.length}
                          </div>
                          {isOpen ? (
                    ...
                    {isOpen && (
                      <div className="px-4 pb-4 bg-[#FAF9F6]">
                        <div className="bg-white border border-[#EFE8DC] overflow-hidden shadow-sm rounded-xl">
                          <div className="max-h-96 overflow-y-auto custom-scroll">
                            {projects.map((row, rowIndex) => (
                              <div
                                key={`${row.name}-${rowIndex}`}
                                className="flex items-center justify-between p-4 border-b border-gray-50 last:border-b-0 gap-3"
                              >
                                <div className="flex items-center gap-4 min-w-0">
                                  <div className={`w-2 h-2 rounded-full ${row.handover === 'N/A' ? 'bg-[#B68A35]/40' : 'bg-[#B68A35]'}`} />
                                  <div className="min-w-0">
                                    <h4 className="font-semibold text-slate-800 text-sm sm:text-[15px] truncate">{row.name}</h4>
                                    <p className="text-slate-400 text-xs">{row.type}</p>
                                  </div>
                                </div>
                                <div className={`font-medium text-sm shrink-0 ${row.handover === 'N/A' ? 'text-slate-400' : 'text-[#B68A35]'}`}>
                                  {row.handover}
                                </div>
                              </div>
                            ))}
                        ...
                        <button
                          type="button"
                          className="relative border border-[#B68A35] w-full h-12 mt-4 flex items-center justify-center rounded-xl hover:bg-[#B68A35]/5 transition-colors"
                        >
            ...
            <div className="bg-white max-h-96 overflow-y-auto custom-scroll overflow-x-auto">
              <table className="w-full min-w-full">
                <thead className="bg-[#FAF9F6] border-b border-[#F2EEE8] sticky top-0">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-sm text-slate-900">Project Name</th>
                    <th className="text-left px-4 py-3 font-semibold text-sm text-slate-900 hidden sm:table-cell">Property Type</th>
                    <th className="text-left px-4 py-3 font-semibold text-sm text-slate-900">Status</th>
                    <th className="text-left px-4 py-3 font-semibold text-sm text-slate-900 hidden lg:table-cell">Location Name</th>
                    <th className="text-left px-4 py-3 font-semibold text-sm text-slate-900 hidden lg:table-cell">Location Subtitle</th>
                  </tr>
                </thead>
                <tbody>
                  {allProjectsList.map((project, idx) => (
                    <tr key={`${project.locationName}-${project.name}-${idx}`} className="border-b border-gray-100 hover:bg-[#FCFAF5] transition-colors last:border-b-0">
                      <td className="px-4 py-3 text-sm text-slate-800 font-medium">{project.name}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 hidden sm:table-cell">{project.type}</td>
                      <td className={`px-4 py-3 text-sm font-medium ${project.handover === 'N/A' ? 'text-slate-400' : 'text-[#B68A35]'}`}>
                        {project.handover}
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-800 font-medium hidden lg:table-cell">{project.locationName}</td>
                      <td className="px-4 py-3 text-sm text-slate-600 hidden lg:table-cell">{project.locationSubtitle}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="mt-5 rounded-2xl flex flex-col lg:flex-row items-start border border-[#F2EEE8] shadow-[0_4px_25px_rgba(0,0,0,0.06)] p-4 sm:p-5 gap-4 bg-white">
          <LegacyCard
            index={15}
...
function FilterPill({ icon: Icon, label }) {
  return (
    <button
      type="button"
      className="bg-white rounded-lg h-12 border border-[#ECE7DE] flex items-center justify-between px-4 hover:bg-[#FBFAF8] lg:w-42.5"
    >
...
function LegacyCard({ index, activeIndex, setActiveIndex, icon: Icon, title, subtitle, items }) {
  ...
  return (
    <div className="bg-[#FAF9F6] rounded-xl overflow-hidden border border-[#F2EEE8] shadow-sm w-full lg:w-1/2">
      <button
        type="button"
        onClick={() => setActiveIndex(isOpen ? null : index)}
        className={`w-full flex p-4 gap-3 items-center text-left transition-colors ${isOpen ? 'bg-[#FAF9F6]' : 'bg-white hover:bg-[#FCFAF5]'}`}
      >
        ...
        <div className="flex justify-between items-center w-full min-w-0">
          <div className="min-w-0">
            <h3 className="font-semibold text-base sm:text-lg text-slate-900 truncate">{title}</h3>
          ...
          <div className="bg-white border border-[#EFE8DC] rounded-xl shadow-sm overflow-hidden">
            <div className="max-h-96 overflow-y-auto custom-scroll">
              {items.map((item, itemIdx) => (
                <div key={`${item.name}-${itemIdx}`} className="flex items-start gap-4 p-4 border-b border-gray-100 last:border-b-0">
